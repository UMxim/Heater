...
#define STM8S_GPIO_H	// Для работы со стандартной библиотекой закомментировать эту строчку



#if !defined (_STM8S_CONFIG_H_)
	#error "Please include first the config STM8S in stm8s_config.h file"
#endif
...
/** @addtogroup ADC1_Registers_Bits_Definition
 ...
 #define ADC1_CSR_CH0     ((uint8_t)0x00) /*!< Channel selection 0 */
 #define ADC1_CSR_CH1     ((uint8_t)0x01) /*!< Channel selection 1 */
 #define ADC1_CSR_CH2     ((uint8_t)0x02) /*!< Channel selection 2 */
 #define ADC1_CSR_CH3     ((uint8_t)0x03) /*!< Channel selection 3 */
 #define ADC1_CSR_CH4     ((uint8_t)0x04) /*!< Channel selection 4 */
 #define ADC1_CSR_CH5     ((uint8_t)0x05) /*!< Channel selection 5 */
 #define ADC1_CSR_CH6     ((uint8_t)0x06) /*!< Channel selection 6 */
 #define ADC1_CSR_CH7     ((uint8_t)0x07) /*!< Channel selection 7 */
 #define ADC1_CSR_CH8     ((uint8_t)0x08) /*!< Channel selection 8 */
 #define ADC1_CSR_CH9     ((uint8_t)0x09) /*!< Channel selection 9 */
 #define ADC1_CSR_CH10    ((uint8_t)0x0A) /*!< Channel selection 10 */
 #define ADC1_CSR_CH11    ((uint8_t)0x0B) /*!< Channel selection 11 */
 #define ADC1_CSR_CH12    ((uint8_t)0x0C) /*!< Channel selection 12 */
 #define ADC1_CSR_CH13    ((uint8_t)0x0D) /*!< Channel selection 13 */
 #define ADC1_CSR_CH14    ((uint8_t)0x0E) /*!< Channel selection 14 */
 #define ADC1_CSR_CH15    ((uint8_t)0x0F) /*!< Channel selection 15 */

 #define ADC1_CR1_SPSEL   ((uint8_t)0x70) /*!< Prescaler selection mask */
 #define ADC1_CR1_SPSEL2  ((uint8_t)0x00) /*!< Prescaler selection F/2 */
 #define ADC1_CR1_SPSEL3  ((uint8_t)0x10) /*!< Prescaler selection F/3 */
 #define ADC1_CR1_SPSEL4  ((uint8_t)0x20) /*!< Prescaler selection F/4 */
 #define ADC1_CR1_SPSEL6  ((uint8_t)0x30) /*!< Prescaler selection F/6 */
 #define ADC1_CR1_SPSEL8  ((uint8_t)0x40) /*!< Prescaler selection F/8 */
 #define ADC1_CR1_SPSEL10 ((uint8_t)0x50) /*!< Prescaler selection F/10 */
 #define ADC1_CR1_SPSEL12 ((uint8_t)0x60) /*!< Prescaler selection F/12 */
 #define ADC1_CR1_SPSEL18 ((uint8_t)0x70) /*!< Prescaler selection F/18 */
 ...
 #define ADC1_CR2_EXTSEL_TIM1 ((uint8_t)0x00) /*!< External event TIM1 */
 #define ADC1_CR2_EXTSEL_ETR  ((uint8_t)0x10) /*!< External event ADC_ETR */
...
#define	CLK_SWR_HSI			 ((uint8_t)0xE1)	// Reset value - HSI clock
#define CLK_SWR_HSE			 ((uint8_t)0xB4)	// HSE clock
#define CLK_SWR_LSI			 ((uint8_t)0xD2)	// LSI clock
...
#define CLK_CKDIVR_HSIDIV8   ((uint8_t)0x18)	// High speed internal clock prescaler 8 - default

#define CLK_CKDIVR_CPUDIV    ((uint8_t)0x07) /*!< CPU clock prescaler */
#define CLK_CKDIVR_CPUDIV1	 ((uint8_t)0x00)	// CPU prescaler == 1 - default
#define CLK_CKDIVR_CPUDIV2	 ((uint8_t)0x01)	// CPU prescaler == 2
#define CLK_CKDIVR_CPUDIV4	 ((uint8_t)0x02)	// CPU prescaler == 4
#define CLK_CKDIVR_CPUDIV8	 ((uint8_t)0x03)	// CPU prescaler == 8
#define CLK_CKDIVR_CPUDIV16	 ((uint8_t)0x04)	// CPU prescaler == 16
#define CLK_CKDIVR_CPUDIV32	 ((uint8_t)0x05)	// CPU prescaler == 32
#define CLK_CKDIVR_CPUDIV64	 ((uint8_t)0x06)	// CPU prescaler == 64
#define CLK_CKDIVR_CPUDIV128 ((uint8_t)0x07)	// CPU prescaler == 128
...
#define CLK_CCOR_CCOSEL_HSIDIV		((uint8_t)0x00)		// CCO = HSI_DIV
#define CLK_CCOR_CCOSEL_LSI			((uint8_t)0x02)		// CCO = LSI
#define CLK_CCOR_CCOSEL_HSE			((uint8_t)0x04)		// CCO = HSE
#define CLK_CCOR_CCOSEL_CPU			((uint8_t)0x08)		// CCO = F_CPU
#define CLK_CCOR_CCOSEL_CPU2		((uint8_t)0x0A)		// CCO = F_CPU / 2
#define CLK_CCOR_CCOSEL_CPU4		((uint8_t)0x0C)		// CCO = F_CPU / 4
#define CLK_CCOR_CCOSEL_CPU8		((uint8_t)0x0E)		// CCO = F_CPU / 8
#define CLK_CCOR_CCOSEL_CPU16		((uint8_t)0x10)		// CCO = F_CPU / 16
#define CLK_CCOR_CCOSEL_CPU32		((uint8_t)0x12)		// CCO = F_CPU / 32
#define CLK_CCOR_CCOSEL_CPU64		((uint8_t)0x14)		// CCO = F_CPU / 64
#define CLK_CCOR_CCOSEL_HSI			((uint8_t)0x16)		// CCO = F_CPU / 16
#define CLK_CCOR_CCOSEL_MASTER		((uint8_t)0x18)		// CCO = MASTER
...
#define TIM1_CR1_CMS_EDGE	((uint8_t)0x00) /*!< edge mode with CR1_DIR */
#define TIM1_CR1_CMS_DOWN	((uint8_t)0x20) /*!< Center-aligned Mode interrupt count down. */
#define TIM1_CR1_CMS_UP		((uint8_t)0x40) /*!< Center-aligned Mode interrupt count up. */
#define TIM1_CR1_CMS_UPDOWH	((uint8_t)0x60) /*!< Center-aligned Mode interrupt count up and down. */
...
#define TIM1_CR2_MMS_RESET	((uint8_t)0x00) /*!< MMS reset. */
#define TIM1_CR2_MMS_ENABLE	((uint8_t)0x10) /*!< MMS enable signal is used as trigger output. */
#define TIM1_CR2_MMS_UPDATE	((uint8_t)0x20) /*!< MMS update event is selected as trigger output. */
#define TIM1_CR2_MMS_MATCH1	((uint8_t)0x30) /*!< MMS trigger output sends a positive pulse. */
#define TIM1_CR2_MMS_OC1REF	((uint8_t)0x40) /*!< MMS OC1REF signal is used as trigger output. */
#define TIM1_CR2_MMS_OC2REF	((uint8_t)0x50) /*!< MMS OC2REF signal is used as trigger output. */
#define TIM1_CR2_MMS_OC3REF	((uint8_t)0x60) /*!< MMS OC3REF signal is used as trigger output. */
#define TIM1_CR2_MMS_OC4REF	((uint8_t)0x70) /*!< MMS OC4REF signal is used as trigger output. */
...
#define TIM1_SMCR_TS_TIM6	((uint8_t)0x00) /* Internal trigger ITR0 connected to TIM6 TRGO */
#define TIM1_SMCR_TS_TIM5	((uint8_t)0x20) /* Internal trigger ITR2 connected to TIM5 TRGO */
#define TIM1_SMCR_TS_TI1FED	((uint8_t)0x40) /* TI1 edge detector (TI1F_ED) */
#define TIM1_SMCR_TS_TI1FP1	((uint8_t)0x50) /* Filtered timer input 1 (TI1FP1) */
#define TIM1_SMCR_TS_TI2FP2	((uint8_t)0x60) /* Filtered timer input 2 (TI2FP2) */
#define TIM1_SMCR_TS_ETRF	((uint8_t)0x70) /* External trigger input (ETRF) */

#define TIM1_SMCR_SMS    ((uint8_t)0x07) /*!< Slave Mode Selection mask. */
#define TIM1_SMCR_SMS_DIS	((uint8_t)0x00) /* Clock/trigger controller disabled */
#define TIM1_SMCR_SMS_ENC1	((uint8_t)0x01) /* Counter counts up or down on TI2FP2 edge depending on TI1FP1 level */
#define TIM1_SMCR_SMS_ENC2	((uint8_t)0x02) /* Counter counts up or down on TI1FP1 edge depending on TI2FP2 level */
#define TIM1_SMCR_SMS_ENC3	((uint8_t)0x03) /* Counter counts up or down on both TI1FP1 and TI2FP2 */
#define TIM1_SMCR_SMS_RESET	((uint8_t)0x04) /* Reset mode */
#define TIM1_SMCR_SMS_GATE	((uint8_t)0x05) /* Trigger gated mode */
#define TIM1_SMCR_SMS_STN	((uint8_t)0x06) /* Trigger standard mode */
#define TIM1_SMCR_SMS_EXT1	((uint8_t)0x07) /* External clock mode */
...
#define TIM1_ETR_ETPS_OFF	((uint8_t)0x00) /*!< Prescaler off */
#define TIM1_ETR_ETPS_2		((uint8_t)0x10) /*!< ETRP frequency divided by 2 */
#define TIM1_ETR_ETPS_4		((uint8_t)0x20) /*!< ETRP frequency divided by 4 */
#define TIM1_ETR_ETPS_8		((uint8_t)0x30) /*!< ETRP frequency divided by 8 */

#define TIM1_ETR_ETF     ((uint8_t)0x0F) /*!< External Trigger Filter mask. */
#define TIM1_ETR_ETF_NO		((uint8_t)0x00) /*!< No filter, sampling is done at fMASTER */
#define TIM1_ETR_ETF_1_2	((uint8_t)0x01) /*!< fSAMPLING=fMASTER, N = 2 */
#define TIM1_ETR_ETF_1_4	((uint8_t)0x02) /*!< fSAMPLING=fMASTER, N = 4 */
#define TIM1_ETR_ETF_1_8	((uint8_t)0x03) /*!< fSAMPLING=fMASTER, N = 8 */
#define TIM1_ETR_ETF_2_6	((uint8_t)0x04) /*!< fSAMPLING=fMASTER/2, N = 6 */
#define TIM1_ETR_ETF_2_8	((uint8_t)0x05) /*!< fSAMPLING=fMASTER/2, N = 8 */
#define TIM1_ETR_ETF_4_6	((uint8_t)0x06) /*!< fSAMPLING=fMASTER/4, N = 6 */
#define TIM1_ETR_ETF_4_8	((uint8_t)0x07) /*!< fSAMPLING=fMASTER/4, N = 8 */
#define TIM1_ETR_ETF_8_6	((uint8_t)0x08) /*!< fSAMPLING=fMASTER/8, N = 6 */
#define TIM1_ETR_ETF_8_8	((uint8_t)0x09) /*!< fSAMPLING=fMASTER/8, N = 8 */
#define TIM1_ETR_ETF_16_5	((uint8_t)0x0A) /*!< fSAMPLING=fMASTER/16, N = 5 */
#define TIM1_ETR_ETF_16_6	((uint8_t)0x0B) /*!< fSAMPLING=fMASTER/16, N = 6 */
#define TIM1_ETR_ETF_16_8	((uint8_t)0x0C) /*!< fSAMPLING=fMASTER/16, N = 8 */
#define TIM1_ETR_ETF_32_5	((uint8_t)0x0D) /*!< fSAMPLING=fMASTER/32, N = 5 */
#define TIM1_ETR_ETF_32_6	((uint8_t)0x0E) /*!< fSAMPLING=fMASTER/32, N = 6 */
#define TIM1_ETR_ETF_32_8	((uint8_t)0x0F) /*!< fSAMPLING=fMASTER/32, N = 8 */
...
#define TIM1_CCMR_ICxF_NO	((uint8_t)0x00) /*!< No filter, fSAMPLING= fMASTER */
#define TIM1_CCMR_ICxF_1_2	((uint8_t)0x10) /*!< fSAMPLING=fMASTER, N = 2 */
#define TIM1_CCMR_ICxF_1_4	((uint8_t)0x20) /*!< fSAMPLING=fMASTER, N = 4 */
#define TIM1_CCMR_ICxF_1_8	((uint8_t)0x30) /*!< fSAMPLING=fMASTER, N = 8 */
#define TIM1_CCMR_ICxF_2_6	((uint8_t)0x40) /*!< fSAMPLING=fMASTER/2, N = 6 */
#define TIM1_CCMR_ICxF_2_8	((uint8_t)0x50) /*!< fSAMPLING=fMASTER/2, N = 8 */
#define TIM1_CCMR_ICxF_4_6	((uint8_t)0x60) /*!< fSAMPLING=fMASTER/4, N = 6 */
#define TIM1_CCMR_ICxF_4_8	((uint8_t)0x70) /*!< fSAMPLING=fMASTER/4, N = 8 */
#define TIM1_CCMR_ICxF_8_6	((uint8_t)0x80) /*!< fSAMPLING=fMASTER/8, N = 6 */
#define TIM1_CCMR_ICxF_8_8	((uint8_t)0x90) /*!< fSAMPLING=fMASTER/8, N = 8 */
#define TIM1_CCMR_ICxF_16_5	((uint8_t)0xA0) /*!< fSAMPLING=fMASTER/16, N = 5 */
#define TIM1_CCMR_ICxF_16_6	((uint8_t)0xB0) /*!< fSAMPLING=fMASTER/16, N = 6 */
#define TIM1_CCMR_ICxF_16_8	((uint8_t)0xC0) /*!< fSAMPLING=fMASTER/16, N = 8 */
#define TIM1_CCMR_ICxF_32_5	((uint8_t)0xD0) /*!< fSAMPLING=fMASTER/32, N = 5 */
#define TIM1_CCMR_ICxF_32_6	((uint8_t)0xE0) /*!< fSAMPLING=fMASTER/32, N = 6 */
#define TIM1_CCMR_ICxF_32_8	((uint8_t)0xF0) /*!< fSAMPLING=fMASTER/32, N = 8 */

#define TIM1_CCMR_ICxPSC ((uint8_t)0x0C) /*!< Input Capture x Prescaler mask. */
#define TIM1_CCMR_ICxPSC_OFF	((uint8_t)0x00) /*!< No prescaler */
#define TIM1_CCMR_ICxPSC_2		((uint8_t)0x04) /*!< Capture is made once every 2 events */
#define TIM1_CCMR_ICxPSC_4		((uint8_t)0x08) /*!< Capture is made once every 4 events */
#define TIM1_CCMR_ICxPSC_8		((uint8_t)0x0C) /*!< Capture is made once every 8 events */

#define TIM1_CCMR_OCxCE  ((uint8_t)0x80)
#define TIM1_CCMR_OCM   ((uint8_t)0x70) /*!< Output Compare x Mode mask. */
#define TIM1_CCMR_OCxM_FROZEN	((uint8_t)0x00) /*!< Output Compare set FROZEN. */
#define TIM1_CCMR_OCxM_ACT_M	((uint8_t)0x10) /*!< Output Compare set ACTIVE if MATCH. */
#define TIM1_CCMR_OCxM_INA_M	((uint8_t)0x20) /*!< Output Compare set INACTIVE if MATCH. */
#define TIM1_CCMR_OCxM_TOGGLE	((uint8_t)0x30) /*!< Output Compare set TOGGLE if MATCH. */
#define TIM1_CCMR_OCxM_INACTIVE	((uint8_t)0x40) /*!< Output Compare set INACTIVE. */
#define TIM1_CCMR_OCxM_ACTIVE	((uint8_t)0x50) /*!< Output Compare set ACTIVE. */
#define TIM1_CCMR_OCxM_PWM1		((uint8_t)0x60) /*!< Output Compare Mode PWM 1. */
#define TIM1_CCMR_OCxM_PWM2		((uint8_t)0x70) /*!< Output Compare Mode PWM 2. */
...
#define TIM1_CCMR_CCxS_OUTPUT  ((uint8_t)0x00)	/* CC1 channel is configured as output */ 
#define TIM1_CCMR_CCxS_IN_TI1  ((uint8_t)0x01)	/* CC1 channel is configured as input, IC1 is mapped on TI1FP1 */ 
#define TIM1_CCMR_CCxS_IN_TI2  ((uint8_t)0x02)	/* CC1 channel is configured as input, IC1 is mapped on TI2FP1 */ 
#define TIM1_CCMR_CCxS_IN_TRC  ((uint8_t)0x03)	/* CC1 channel is configured as input, IC1 is mapped on TRC */ 
...
/*IER*/
#define TIM2_IER_TIE     ((uint8_t)0x40) /*!< Trigger interrupt Enable mask. */
...
/*EGR*/
#define TIM2_EGR_TG      ((uint8_t)0x40) /*!< Trigger Generation mask. */
...
#define TIM2_CCMR_OCxM_FROZEN	((uint8_t)0x00) /*!< Output Compare set FROZEN. */
#define TIM2_CCMR_OCxM_ACT_M	((uint8_t)0x10) /*!< Output Compare set ACTIVE if MATCH. */
#define TIM2_CCMR_OCxM_INA_M	((uint8_t)0x20) /*!< Output Compare set INACTIVE if MATCH. */
#define TIM2_CCMR_OCxM_TOGGLE	((uint8_t)0x30) /*!< Output Compare set TOGGLE if MATCH. */
#define TIM2_CCMR_OCxM_INACTIVE	((uint8_t)0x40) /*!< Output Compare set INACTIVE. */
#define TIM2_CCMR_OCxM_ACTIVE	((uint8_t)0x50) /*!< Output Compare set ACTIVE. */
#define TIM2_CCMR_OCxM_PWM1		((uint8_t)0x60) /*!< Output Compare Mode PWM 1. */
#define TIM2_CCMR_OCxM_PWM2		((uint8_t)0x70) /*!< Output Compare Mode PWM 2. */
#define TIM2_CCMR_CCxS_OUTPUT	((uint8_t)0x00) // Set channel as output
#define TIM2_CCMR_CCxS_INPUT1	((uint8_t)0x01) // Set channel as input from TI1FP1
#define TIM2_CCMR_CCxS_INPUT2	((uint8_t)0x02) // Set channel as input from TI2FP1
#define TIM2_CCMR_CCxS_IN_TRC	((uint8_t)0x03) // Set channel as input from TRC


#define TIM2_CCMR_OCxM0  ((uint8_t)0x00)
#define TIM2_CCMR_OCxM1  ((uint8_t)0x10)
#define TIM2_CCMR_OCxM2  ((uint8_t)0x20)
#define TIM2_CCMR_OCxM3  ((uint8_t)0x30)
#define TIM2_CCMR_OCxM4  ((uint8_t)0x40)
#define TIM2_CCMR_OCxM5  ((uint8_t)0x50)
#define TIM2_CCMR_OCxM6  ((uint8_t)0x60)
#define TIM2_CCMR_OCxM7  ((uint8_t)0x70)
...
#define TIM2_CCMR_CCxS0  ((uint8_t)0x00) 
#define TIM2_CCMR_CCxS1  ((uint8_t)0x01) 
#define TIM2_CCMR_CCxS2  ((uint8_t)0x02) 
#define TIM2_CCMR_CCxS3  ((uint8_t)0x03) 
/*CCER1*/
...
/*PSCR*/
#define TIM2_PSCR_PSC    ((uint8_t)0x0F) /*!< Prescaler Value (MSB) mask. */
#define TIM2_PSCR_PSC0   ((uint8_t)0x00) /*!< Prescaler Value (MSB) mask. */
#define TIM2_PSCR_PSC2   ((uint8_t)0x01) /*!< Prescaler Value (MSB) mask. */
#define TIM2_PSCR_PSC4   ((uint8_t)0x02) /*!< Prescaler Value (MSB) mask. */
#define TIM2_PSCR_PSC8   ((uint8_t)0x03) /*!< Prescaler Value (MSB) mask. */
#define TIM2_PSCR_PSC16  ((uint8_t)0x04) /*!< Prescaler Value (MSB) mask. */
#define TIM2_PSCR_PSC32  ((uint8_t)0x05) /*!< Prescaler Value (MSB) mask. */
#define TIM2_PSCR_PSC64  ((uint8_t)0x06) /*!< Prescaler Value (MSB) mask. */
#define TIM2_PSCR_PSC128    ((uint8_t)0x07) /*!< Prescaler Value (MSB) mask. */
#define TIM2_PSCR_PSC256    ((uint8_t)0x08) /*!< Prescaler Value (MSB) mask. */
#define TIM2_PSCR_PSC1024   ((uint8_t)0x09) /*!< Prescaler Value (MSB) mask. */
#define TIM2_PSCR_PSC2048   ((uint8_t)0x0A) /*!< Prescaler Value (MSB) mask. */
#define TIM2_PSCR_PSC4096   ((uint8_t)0x0B) /*!< Prescaler Value (MSB) mask. */
#define TIM2_PSCR_PSC8192   ((uint8_t)0x0C) /*!< Prescaler Value (MSB) mask. */
#define TIM2_PSCR_PSC16536  ((uint8_t)0x0E) /*!< Prescaler Value (MSB) mask. */
#define TIM2_PSCR_PSC32768  ((uint8_t)0x0F) /*!< Prescaler Value (MSB) mask. */
/*ARR*/
...
#define TIM3_CCMR_OCxM0  ((uint8_t)0x00)
#define TIM3_CCMR_OCxM1  ((uint8_t)0x10)
#define TIM3_CCMR_OCxM2  ((uint8_t)0x20)
#define TIM3_CCMR_OCxM3  ((uint8_t)0x30)
#define TIM3_CCMR_OCxM4  ((uint8_t)0x40)
#define TIM3_CCMR_OCxM5  ((uint8_t)0x50)
#define TIM3_CCMR_OCxM6  ((uint8_t)0x60)
#define TIM3_CCMR_OCxM7  ((uint8_t)0x70)
...
#define TIM3_CCMR_CCxS0  ((uint8_t)0x00) 
#define TIM3_CCMR_CCxS1  ((uint8_t)0x01) 
#define TIM3_CCMR_CCxS2  ((uint8_t)0x02) 
#define TIM3_CCMR_CCxS3  ((uint8_t)0x03) 
/*CCER1*/
...
/*PSCR*/
#define TIM3_PSCR_PSC    ((uint8_t)0x0F) /*!< Prescaler Value (MSB) mask. */
#define TIM3_PSCR_PSC0   ((uint8_t)0x00) /*!< Prescaler Value (MSB) mask. */
#define TIM3_PSCR_PSC2   ((uint8_t)0x01) /*!< Prescaler Value (MSB) mask. */
#define TIM3_PSCR_PSC4   ((uint8_t)0x02) /*!< Prescaler Value (MSB) mask. */
#define TIM3_PSCR_PSC8   ((uint8_t)0x03) /*!< Prescaler Value (MSB) mask. */
#define TIM3_PSCR_PSC16  ((uint8_t)0x04) /*!< Prescaler Value (MSB) mask. */
#define TIM3_PSCR_PSC32  ((uint8_t)0x05) /*!< Prescaler Value (MSB) mask. */
#define TIM3_PSCR_PSC64  ((uint8_t)0x06) /*!< Prescaler Value (MSB) mask. */
#define TIM3_PSCR_PSC128    ((uint8_t)0x07) /*!< Prescaler Value (MSB) mask. */
#define TIM3_PSCR_PSC256    ((uint8_t)0x08) /*!< Prescaler Value (MSB) mask. */
#define TIM3_PSCR_PSC1024   ((uint8_t)0x09) /*!< Prescaler Value (MSB) mask. */
#define TIM3_PSCR_PSC2048   ((uint8_t)0x0A) /*!< Prescaler Value (MSB) mask. */
#define TIM3_PSCR_PSC4096   ((uint8_t)0x0B) /*!< Prescaler Value (MSB) mask. */
#define TIM3_PSCR_PSC8192   ((uint8_t)0x0C) /*!< Prescaler Value (MSB) mask. */
#define TIM3_PSCR_PSC16536  ((uint8_t)0x0E) /*!< Prescaler Value (MSB) mask. */
#define TIM3_PSCR_PSC32768  ((uint8_t)0x0F) /*!< Prescaler Value (MSB) mask. */
/*ARR*/
...
#define TIM4_PSCR_PSC1   ((uint8_t)0x00)	// делитель на 1
#define TIM4_PSCR_PSC2   ((uint8_t)0x01)	// делитель на 2
#define TIM4_PSCR_PSC4   ((uint8_t)0x02)	// делитель на 4
#define TIM4_PSCR_PSC8   ((uint8_t)0x03)	// делитель на 8
#define TIM4_PSCR_PSC16  ((uint8_t)0x04)	// делитель на 16
#define TIM4_PSCR_PSC32  ((uint8_t)0x05)	// делитель на 32
#define TIM4_PSCR_PSC64	 ((uint8_t)0x06)	// делитель на 64
#define TIM4_PSCR_PSC128 ((uint8_t)0x07)	// делитель на 128
...
#define	ITC_SPR1_AWU_MSK		((uint8_t)0x0C)
#define	ITC_SPR1_AWU_LVL1		((uint8_t)0x04)
#define	ITC_SPR1_AWU_LVL2		((uint8_t)0x00)
#define	ITC_SPR1_AWU_LVL3		((uint8_t)0x0C)

#define	ITC_SPR1_CLK_MSK		((uint8_t)0x30)
#define	ITC_SPR1_CLK_LVL1		((uint8_t)0x10)
#define	ITC_SPR1_CLK_LVL2		((uint8_t)0x00)
#define	ITC_SPR1_CLK_LVL3		((uint8_t)0x30)

#define	ITC_SPR1_PORTA_MSK		((uint8_t)0xC0)
#define	ITC_SPR1_PORTA_LVL1		((uint8_t)0x40)
#define	ITC_SPR1_PORTA_LVL2		((uint8_t)0x00)
#define	ITC_SPR1_PORTA_LVL3		((uint8_t)0xC0)

#define	ITC_SPR2_PORTB_MSK		((uint8_t)0x03)
#define	ITC_SPR2_PORTB_LVL1		((uint8_t)0x01)
#define	ITC_SPR2_PORTB_LVL2		((uint8_t)0x00)
#define	ITC_SPR2_PORTB_LVL3		((uint8_t)0x03)

#define	ITC_SPR2_PORTC_MSK		((uint8_t)0x0C)
#define	ITC_SPR2_PORTC_LVL1		((uint8_t)0x04)
#define	ITC_SPR2_PORTC_LVL2		((uint8_t)0x00)
#define	ITC_SPR2_PORTC_LVL3		((uint8_t)0x0C)

#define	ITC_SPR2_PORTD_MSK		((uint8_t)0x30)
#define	ITC_SPR2_PORTD_LVL1		((uint8_t)0x10)
#define	ITC_SPR2_PORTD_LVL2		((uint8_t)0x00)
#define	ITC_SPR2_PORTD_LVL3		((uint8_t)0x30)

#define	ITC_SPR2_PORTE_MSK		((uint8_t)0xC0)
#define	ITC_SPR2_PORTE_LVL1		((uint8_t)0x40)
#define	ITC_SPR2_PORTE_LVL2		((uint8_t)0x00)
#define	ITC_SPR2_PORTE_LVL3		((uint8_t)0xC0)

#define	ITC_SPR3_VECT8_MSK		((uint8_t)0x03)
#define	ITC_SPR3_VECT8_LVL1		((uint8_t)0x01)
#define	ITC_SPR3_VECT8_LVL2		((uint8_t)0x00)
#define	ITC_SPR3_VECT8_LVL3		((uint8_t)0x03)

#define	ITC_SPR3_VECT9_MSK		((uint8_t)0x0C)
#define	ITC_SPR3_VECT9_LVL1		((uint8_t)0x04)
#define	ITC_SPR3_VECT9_LVL2		((uint8_t)0x00)
#define	ITC_SPR3_VECT9_LVL3		((uint8_t)0x0C)

#define	ITC_SPR3_SPI_MSK		((uint8_t)0x30)
#define	ITC_SPR3_SPI_LVL1		((uint8_t)0x10)
#define	ITC_SPR3_SPI_LVL2		((uint8_t)0x00)
#define	ITC_SPR3_SPI_LVL3		((uint8_t)0x30)

#define	ITC_SPR3_TIM1OVF_MSK	((uint8_t)0xC0)
#define	ITC_SPR3_TIM1OVF_LVL1	((uint8_t)0x40)
#define	ITC_SPR3_TIM1OVF_LVL2	((uint8_t)0x00)
#define	ITC_SPR3_TIM1OVF_LVL3	((uint8_t)0xC0)

#define	ITC_SPR4_TIM1CC_MSK		((uint8_t)0x03)
#define	ITC_SPR4_TIM1CC_LVL1	((uint8_t)0x01)
#define	ITC_SPR4_TIM1CC_LVL2	((uint8_t)0x00)
#define	ITC_SPR4_TIM1CC_LVL3	((uint8_t)0x03)

#define	ITC_SPR4_TIM2OVF_MSK	((uint8_t)0x0C)
#define	ITC_SPR4_TIM2OVF_LVL1	((uint8_t)0x04)
#define	ITC_SPR4_TIM2OVF_LVL2	((uint8_t)0x00)
#define	ITC_SPR4_TIM2OVF_LVL3	((uint8_t)0x0C)

#define	ITC_SPR4_TIM2CC_MSK		((uint8_t)0x30)
#define	ITC_SPR4_TIM2CC_LVL1	((uint8_t)0x10)
#define	ITC_SPR4_TIM2CC_LVL2	((uint8_t)0x00)
#define	ITC_SPR4_TIM2CC_LVL3	((uint8_t)0x30)

#define	ITC_SPR4_VECT15_MSK		((uint8_t)0xC0)
#define	ITC_SPR4_VECT15_LVL1	((uint8_t)0x40)
#define	ITC_SPR4_VECT15_LVL2	((uint8_t)0x00)
#define	ITC_SPR4_VECT15_LVL3	((uint8_t)0xC0)

#define	ITC_SPR5_VECT16_MSK		((uint8_t)0x03)
#define	ITC_SPR5_VECT16_LVL1	((uint8_t)0x01)
#define	ITC_SPR5_VECT16_LVL2	((uint8_t)0x00)
#define	ITC_SPR5_VECT16_LVL3	((uint8_t)0x03)

#define	ITC_SPR5_UART1TX_MSK	((uint8_t)0x0C)
#define	ITC_SPR5_UART1TX_LVL1	((uint8_t)0x04)
#define	ITC_SPR5_UART1TX_LVL2	((uint8_t)0x00)
#define	ITC_SPR5_UART1TX_LVL3	((uint8_t)0x0C)

#define	ITC_SPR5_UART1RX_MSK	((uint8_t)0x30)
#define	ITC_SPR5_UART1RX_LVL1	((uint8_t)0x10)
#define	ITC_SPR5_UART1RX_LVL2	((uint8_t)0x00)
#define	ITC_SPR5_UART1RX_LVL3	((uint8_t)0x30)

#define	ITC_SPR5_I2C_MSK		((uint8_t)0xC0)
#define	ITC_SPR5_I2C_LVL1		((uint8_t)0x40)
#define	ITC_SPR5_I2C_LVL2		((uint8_t)0x00)
#define	ITC_SPR5_I2C_LVL3		((uint8_t)0xC0)

#define	ITC_SPR6_VECT20_MSK		((uint8_t)0x03)
#define	ITC_SPR6_VECT20_LVL1	((uint8_t)0x01)
#define	ITC_SPR6_VECT20_LVL2	((uint8_t)0x00)
#define	ITC_SPR6_VECT20_LVL3	((uint8_t)0x03)

#define	ITC_SPR6_VECT21_MSK		((uint8_t)0x0C)
#define	ITC_SPR6_VECT21_LVL1	((uint8_t)0x04)
#define	ITC_SPR6_VECT21_LVL2	((uint8_t)0x00)
#define	ITC_SPR6_VECT21_LVL3	((uint8_t)0x0C)

#define	ITC_SPR6_ADC1_MSK		((uint8_t)0x30)
#define	ITC_SPR6_ADC1_LVL1		((uint8_t)0x10)
#define	ITC_SPR6_ADC1_LVL2		((uint8_t)0x00)
#define	ITC_SPR6_ADC1_LVL3		((uint8_t)0x30)

#define	ITC_SPR6_TIM4OVF_MSK	((uint8_t)0xC0)
#define	ITC_SPR6_TIM4OVF_LVL1	((uint8_t)0x40)
#define	ITC_SPR6_TIM4OVF_LVL2	((uint8_t)0x00)
#define	ITC_SPR6_TIM4OVF_LVL3	((uint8_t)0xC0)

#define	ITC_SPR7_FLASH_MSK		((uint8_t)0x33)
#define	ITC_SPR7_FLASH_LVL1		((uint8_t)0x01)
#define	ITC_SPR7_FLASH_LVL2		((uint8_t)0x00)
#define	ITC_SPR7_FLASH_LVL3		((uint8_t)0x03)
...
#define EXTI_CR1_PDIS_MASK	((uint8_t)0xC0) /*!< PORTD external interrupt sensitivity bits mask */
#define EXTI_CR1_PDIS_FL	((uint8_t)0x00) /*!< PORTD external interrupt Falling edge and Low level */
#define EXTI_CR1_PDIS_R		((uint8_t)0x40) /*!< PORTD external interrupt Rising edge only */
#define EXTI_CR1_PDIS_F		((uint8_t)0x80) /*!< PORTD external interrupt Falling edge only */
#define EXTI_CR1_PDIS_FR	((uint8_t)0xC0) /*!< PORTD external interrupt Rising and Falling edge */

#define EXTI_CR1_PCIS_MASK	((uint8_t)0x30) /*!< PORTC external interrupt sensitivity bits mask */
#define EXTI_CR1_PCIS_FL	((uint8_t)0x00) /*!< PORTC external interrupt Falling edge and Low level */
#define EXTI_CR1_PCIS_R		((uint8_t)0x10) /*!< PORTC external interrupt Rising edge only */
#define EXTI_CR1_PCIS_F		((uint8_t)0x20) /*!< PORTC external interrupt Falling edge only */
#define EXTI_CR1_PCIS_FR	((uint8_t)0x30) /*!< PORTC external interrupt Rising and Falling edge */

#define EXTI_CR1_PBIS_MASK	((uint8_t)0x0C) /*!< PORTB external interrupt sensitivity bits mask */
#define EXTI_CR1_PBIS_FL	((uint8_t)0x00) /*!< PORTB external interrupt Falling edge and Low level */
#define EXTI_CR1_PBIS_R		((uint8_t)0x04) /*!< PORTB external interrupt Rising edge only */
#define EXTI_CR1_PBIS_F		((uint8_t)0x08) /*!< PORTB external interrupt Falling edge only */
#define EXTI_CR1_PBIS_FR	((uint8_t)0x0C) /*!< PORTB external interrupt Rising and Falling edge */

#define EXTI_CR1_PAIS_MASK	((uint8_t)0x03) /*!< PORTA external interrupt sensitivity bits mask */
#define EXTI_CR1_PAIS_FL	((uint8_t)0x00) /*!< PORTA external interrupt Falling edge and Low level */
#define EXTI_CR1_PAIS_R		((uint8_t)0x01) /*!< PORTA external interrupt Rising edge only */
#define EXTI_CR1_PAIS_F		((uint8_t)0x02) /*!< PORTA external interrupt Falling edge only */
#define EXTI_CR1_PAIS_FR	((uint8_t)0x03) /*!< PORTA external interrupt Rising and Falling edge */

#define EXTI_CR2_TLIS		((uint8_t)0x04) /*!< Top level interrupt sensitivity bit mask */

#define EXTI_CR2_PEIS_MASK	((uint8_t)0x03) /*!< PORTE external interrupt sensitivity bits mask */
#define EXTI_CR1_PEIS_FL	((uint8_t)0x00) /*!< PORTE external interrupt Falling edge and Low level */
#define EXTI_CR1_PEIS_R		((uint8_t)0x01) /*!< PORTE external interrupt Rising edge only */
#define EXTI_CR1_PEIS_F		((uint8_t)0x02) /*!< PORTE external interrupt Falling edge only */
#define EXTI_CR1_PEIS_FR	((uint8_t)0x03) /*!< PORTE external interrupt Rising and Falling edge */
...
#define FLASH_PUKR_KEY1       ((uint8_t)0x56) /*!< Flash Program memory unprotection program word 1 */
#define FLASH_PUKR_KEY2       ((uint8_t)0xAE) /*!< Flash Program memory unprotection program word 2 */

#define FLASH_DUKR_DUK        ((uint8_t)0xFF) /*!< Data EEPROM unprotection mask */
#define FLASH_DUKR_KEY1       ((uint8_t)0xAE) /*!< Data EEPROM unprotection eeprom word 1 */
#define FLASH_DUKR_KEY2       ((uint8_t)0x56) /*!< Data EEPROM unprotection eeprom word 2 */
...
#define OPT_OPT0_ROP	((uint8_t)0xAA) /*!< OPT Enable readout protection */

#define OPT_OPT2_AFR7	((uint8_t)0x80) /*!< OPT2 AFR7 */
#define OPT_OPT2_AFR6	((uint8_t)0x40) /*!< OPT2 AFR6 */
#define OPT_OPT2_AFR5	((uint8_t)0x20) /*!< OPT2 AFR5 */
#define OPT_OPT2_AFR4	((uint8_t)0x10) /*!< OPT2 AFR4 */
#define OPT_OPT2_AFR3	((uint8_t)0x08) /*!< OPT2 AFR3 */
#define OPT_OPT2_AFR2	((uint8_t)0x04) /*!< OPT2 AFR2 */
#define OPT_OPT2_AFR1	((uint8_t)0x02) /*!< OPT2 AFR1 */
#define OPT_OPT2_AFR0	((uint8_t)0x01) /*!< OPT2 AFR0 */

#define OPT_NOPT2_NAFR7	((uint8_t)~0x80) /*!< NOPT2 NAFR7 */
#define OPT_NOPT2_NAFR6	((uint8_t)~0x40) /*!< NOPT2 NAFR6 */
#define OPT_NOPT2_NAFR5	((uint8_t)~0x20) /*!< NOPT2 NAFR5 */
#define OPT_NOPT2_NAFR4	((uint8_t)~0x10) /*!< NOPT2 NAFR4 */
#define OPT_NOPT2_NAFR3	((uint8_t)~0x08) /*!< NOPT2 NAFR3 */
#define OPT_NOPT2_NAFR2	((uint8_t)~0x04) /*!< NOPT2 NAFR2 */
#define OPT_NOPT2_NAFR1	((uint8_t)~0x02) /*!< NOPT2 NAFR1 */
#define OPT_NOPT2_NAFR0	((uint8_t)~0x01) /*!< NOPT2 NAFR0 */

#define OPT_OPT3_HSI_TRIM	((uint8_t)0x10) /*!< OPT3 HSI_TRIM */
#define OPT_OPT3_LSI_EN		((uint8_t)0x08) /*!< OPT3 LSI_EN */
#define OPT_OPT3_IWDG_HW	((uint8_t)0x04) /*!< OPT3 IWDG_HW */
#define OPT_OPT3_WWDG_HW	((uint8_t)0x02) /*!< OPT3 WWDG_HW */
#define OPT_OPT3_WWDG_HALT	((uint8_t)0x01) /*!< OPT3 WWDG_HALT */

#define OPT_NOPT3_NHSI_TRIM	((uint8_t)~0x10) /*!< NOPT3 NHSI_TRIM */
#define OPT_NOPT3_NLSI_EN	((uint8_t)~0x08) /*!< NOPT3 NLSI_EN */
#define OPT_NOPT3_NIWDG_HW	((uint8_t)~0x04) /*!< NOPT3 NIWDG_HW */
#define OPT_NOPT3_NWWDG_HW	((uint8_t)~0x02) /*!< NOPT3 NWWDG_HW */
#define OPT_NOPT3_NWWDG_HALT	((uint8_t~)0x01) /*!< NOPT3 NWWDG_HALT */

#define OPT_OPT4_EXT_CLK	((uint8_t)0x08) /*!< OPT3 EXT_CLK */
#define OPT_OPT4_CKAWU_SEL	((uint8_t)0x04) /*!< OPT3 CKAWU_SEL */
#define OPT_OPT4_PRS_C1		((uint8_t)0x02) /*!< OPT3 PRS_C1 */
#define OPT_OPT4_PRS_C0		((uint8_t)0x01) /*!< OPT3 PRS_C0 */

#define OPT_NOPT4_NEXT_CLK		((uint8_t)~0x08) /*!< NOPT3 NEXT_CLK */
#define OPT_NOPT4_NCKAWU_SEL	((uint8_t)~0x04) /*!< NOPT3 NCKAWU_SEL */
#define OPT_NOPT4_NPRS_C1		((uint8_t)~0x02) /*!< NOPT3 NPRS_C1 */
#define OPT_NOPT4_NPRS_C0		((uint8_t)~0x01) /*!< NOPT3 NPRS_C0 */
...
/** @addtogroup IWDG_Registers_Reset_Value
  * @{
  */
#define IWDG_KR_KEY_ENABLE		((uint8_t)0xCC)	// разрешает вочдог
#define IWDG_KR_KEY_REFRESH		((uint8_t)0xAA)	// сбрасывает вочдог
#define IWDG_KR_KEY_ACCESS		((uint8_t)0x55)	// разрешает запись PR RLR

#define IWDG_PR_RESET_VALUE		((uint8_t)0x00)
#define IWDG_PR_4				((uint8_t)0x00)	// divider LSI128k / 4
#define IWDG_PR_8				((uint8_t)0x01)	// divider LSI128k / 8
#define IWDG_PR_16				((uint8_t)0x02)	// divider LSI128k / 16
#define IWDG_PR_32				((uint8_t)0x03)	// divider LSI128k / 32
#define IWDG_PR_64				((uint8_t)0x04)	// divider LSI128k / 64
#define IWDG_PR_128				((uint8_t)0x05)	// divider LSI128k / 128
#define IWDG_PR_256				((uint8_t)0x06)	// divider LSI128k / 256
...
#define	RST_SR_MASK		((uint8_t)0x1F)	/* RESET flag mask */
...
#define SPI_CR1_LSBFIRST ((uint8_t)0x80) /*!< Frame format LSB bit first */
#define SPI_CR1_MSBFIRST ((uint8_t)0x00) /*!< Frame format MSB bit first */

#define SPI_CR1_SPE      ((uint8_t)0x40) /*!< Enable bits mask */

#define SPI_CR1_BR       ((uint8_t)0x38) /*!< Baud rate control mask */
#define SPI_CR1_BR_F_2   ((uint8_t)0x00) /*!< Baud rate Fmaster/2 */
#define SPI_CR1_BR_F_4   ((uint8_t)0x08) /*!< Baud rate Fmaster/4 */
#define SPI_CR1_BR_F_8   ((uint8_t)0x10) /*!< Baud rate Fmaster/8 */
#define SPI_CR1_BR_F_16  ((uint8_t)0x18) /*!< Baud rate Fmaster/16 */
#define SPI_CR1_BR_F_32  ((uint8_t)0x20) /*!< Baud rate Fmaster/32 */
#define SPI_CR1_BR_F_64  ((uint8_t)0x28) /*!< Baud rate Fmaster/64 */
#define SPI_CR1_BR_F_128 ((uint8_t)0x30) /*!< Baud rate Fmaster/128 */
#define SPI_CR1_BR_F_256 ((uint8_t)0x38) /*!< Baud rate Fmaster/256 */
...
#define	UART1_BRR1_VALUE	((uint8_t)((((F_MASTER + (UART_BAUDRATE >> 1)) / UART_BAUDRATE) >> 4) & 0xFF))
#define UART1_BRR2_VALUE	((uint8_t)((((F_MASTER + (UART_BAUDRATE >> 1)) / UART_BAUDRATE) >> 8) & 0xF0) \
							| (((F_MASTER + (UART_BAUDRATE / 2)) / UART_BAUDRATE) & 0x0F))
...
#define UART1_CR1_9BIT    ((uint8_t)0x10) /*!< Word length mask */
#define UART1_CR1_WAKE    ((uint8_t)0x08) /*!< Wake-up method mask */
#define UART1_CR1_PCEN    ((uint8_t)0x04) /*!< Parity Control Enable mask */
#define UART1_CR1_PARITY_NO   ((uint8_t)0x00)      // No Parity
#define UART1_CR1_PARITY_EVEN ((uint8_t)0x04)      // Even Parity
#define UART1_CR1_PARITY_ODD  ((uint8_t)0x06)      // Odd Parity
#define UART1_CR1_PCEN    ((uint8_t)0x04) /*!< Parity Control Enable mask */
...
#define UART1_CR3_STOP1   ((uint8_t)0x00)	// 1 STOP bit
#define UART1_CR3_STOP15  ((uint8_t)0x30)	// 1.5 STOP bit
#define UART1_CR3_STOP2   ((uint8_t)0x20)	// 2 STOP bit
...
#define UART2_CR1_PARITY_NO   ((uint8_t)0x00)      /**< No Parity*/
#define UART2_CR1_PARITY_EVEN ((uint8_t)0x04)      /**< Even Parity*/
#define UART2_CR1_PARITY_ODD  ((uint8_t)0x06)       /**< Odd Parity*/
...
#define UART2_CR3_STOP1   ((uint8_t)0x00)	// 1 STOP bit
#define UART2_CR3_STOP15  ((uint8_t)0x30)	// 1.5 STOP bit
#define UART2_CR3_STOP2   ((uint8_t)0x20)	// 2 STOP bit
...
#ifndef STM8S_GPIO_H /* Для совместимости с внешней библиотекой stm8s_gpio.h */
...
#else				/* Для совместимости с внешней библиотекой stm8s_gpio.h */
					
/*
ver 0.2b  gpio.h
Назначение - дать удобный доступ к портам ввода/вывода MCU серии STM8S
TODO - исправить ошибки, если найдутся, проверить (адаптировать) с другими компиляторами.
Отзывы, замечания присылать на kison@mail.ru
Обновления/замечания/ исправление ошибок - с этим сюда - http://open-electronics.ru/forum
Обновлено и дополнено Петрухин Илья 2014.
*/


/* Функция конфигурации вывода как выхода
@inline void GPIO_ConfigOutput(gpio_t *gpio, uint8_t pin, OutputSpeed_t speed, OutputType_t type);
	Первый параметр - указатель на структуру GPIO, например GPIOA,GPIOB и т.д.
	Второй параметр - номер конфигурируемого разряда, 0-7.
	Третий параметр - скорость вывода. Принимается PortLoSpeed или PortHiSpeed
	Четвертый параметр - тип выхода. Может быть PortOpenDrain - открытый сток или
                                                PortPushPull - нормальный двухтактный выход.
    Пример:
    GPIO_ConfigOutput (GPIOE, 0, PortLoSpeed, PortOpenDrain);
*/


/* Функция конфигурации вывода как входа
@inline void GPIO_ConfigInput(gpio_t *gpio, uint8_t pin, PinInputType_t type, PinInputIrq_t  irq)
    Первый параметр - указатель на структуру GPIO, например GPIOA,GPIOB и т.д.
    Второй параметр - номер конфигурируемого разряда, 0-7
    Третий параметр - режим входа, может быть подтянутый резистором PinPullup или свободным - PinFloating
    Четвертый параметр - вкл/выкл прерываний от входа. Может быть PinIrqOn или PinIrqOff
*/


/* Структура порта:
    Выходной регистр данных  - GPIOx_ODR или PORTx. Пример PORTA, GPIOB_ODR
    Входной регистр  - GPIOx_IDR или PINx. Пример PINA, GPIOB_IDR
    Регистр направления - GPIOx_DDR или DDRx. Пример DDRA, GPIOB_DDR
    Разряды этих регистров доступны индивидуально:
    пример - PORTA0, PORTB6, PIND4, DDRB6
    Конфигурационные регистры CR1 и CR2 доступны через указатель на GPIO
    Пример GPIOA->CR1 |= (1<<bit_num);
    Также доступ можно получить через GPIOx_ - GPIOB_CR1
    Пример - GPIOA_CR1 = 0;
    Возможен доступ и к отдельным разрядам регистров CR1/CR2 пример - GPIOA->CR1_BITS.bit0 = 1;
*/
    
    typedef enum
    {
        PortLoSpeed         = ((uint8_t)0x00),
        PortHiSpeed         = ((uint8_t)0x01) 
    } OutputSpeed_t;
    
    typedef enum
    {
        PortOpenDrain       = ((uint8_t)0x00),
        PortPushPull	    = ((uint8_t)0x01) 
    } OutputType_t;
    
    typedef enum
    {
        PinFloating        = ((uint8_t)0x00),
        PinPullup          = ((uint8_t)0x01) 
    } InputType_t;
    
    typedef enum
    {
        PinIrqOn 		    = ((uint8_t)0x00),
        PinIrqOff  	    	= ((uint8_t)0x01) 
    } InputIrq_t;
        ...
        union
            {
                ...
                volatile bits_t     ODR_BITS;
            };
        union
            {
                volatile uint8_t    IDR;
                volatile bits_t     IDR_BITS;
            };
        union
            {
                volatile uint8_t    DDR;
                volatile bits_t     DDR_BITS;
            };
        union
            {
                volatile uint8_t    CR1;
                volatile bits_t     CR1_BITS;
            };
        union
            {
                volatile uint8_t    CR2;
                volatile bits_t     CR2_BITS;
            };
    } gpio_t;
    
    #define GPIOA	    ((gpio_t*)GPIOA_BaseAddress)
    #define	GPIOB	    ((gpio_t*)GPIOB_BaseAddress)
    #define	GPIOC	    ((gpio_t*)GPIOC_BaseAddress)
    #define	GPIOD	    ((gpio_t*)GPIOD_BaseAddress)
    #define	GPIOE	    ((gpio_t*)GPIOE_BaseAddress)
    #define	GPIOF	    ((gpio_t*)GPIOF_BaseAddress)
    #define	GPIOG	    ((gpio_t*)GPIOG_BaseAddress)
    #define	GPIOH	    ((gpio_t*)GPIOH_BaseAddress)
    #define	GPIOI	    ((gpio_t*)GPIOI_BaseAddress)
    
    #define GPIOA_CR1   (GPIOA->CR1)
    #define GPIOA_CR2   (GPIOA->CR2)

    #define GPIOB_CR1   (GPIOB->CR1)
    #define GPIOB_CR2   (GPIOB->CR2)

    #define GPIOC_CR1   (GPIOC->CR1)
    #define GPIOC_CR2   (GPIOC->CR2)

    #define GPIOD_CR1   (GPIOD->CR1)
    #define GPIOD_CR2   (GPIOD->CR2)
    
    #define GPIOE_CR1   (GPIOE->CR1)
    #define GPIOE_CR2   (GPIOE->CR2)

    #define GPIOF_CR1   (GPIOF->CR1)
    #define GPIOF_CR2   (GPIOF->CR2)
    
    #define GPIOG_CR1   (GPIOG->CR1)
    #define GPIOG_CR2   (GPIOG->CR2)    

    #define GPIOH_CR1   (GPIOH->CR1)
    #define GPIOH_CR2   (GPIOH->CR2)

    #define GPIOI_CR1   (GPIOI->CR1)
    #define GPIOI_CR2   (GPIOI->CR2)

    #define PORTA       (GPIOA->ODR)
    #define GPIOA_ODR   (GPIOA->ODR)
    #define PORTA0      (GPIOA->ODR_BITS.bit0)
    #define PORTA1      (GPIOA->ODR_BITS.bit1)
    #define PORTA2      (GPIOA->ODR_BITS.bit2)
    #define PORTA3      (GPIOA->ODR_BITS.bit3)
    #define PORTA4      (GPIOA->ODR_BITS.bit4)
    #define PORTA5      (GPIOA->ODR_BITS.bit5)
    #define PORTA6      (GPIOA->ODR_BITS.bit6)
    #define PORTA7      (GPIOA->ODR_BITS.bit7)
    
    #define PORTB       (GPIOB->ODR)
    #define GPIOB_ODR   (GPIOB->ODR)
    #define PORTB0      (GPIOB->ODR_BITS.bit0)
    #define PORTB1      (GPIOB->ODR_BITS.bit1)
    #define PORTB2      (GPIOB->ODR_BITS.bit2)
    #define PORTB3      (GPIOB->ODR_BITS.bit3)
    #define PORTB4      (GPIOB->ODR_BITS.bit4)
    #define PORTB5      (GPIOB->ODR_BITS.bit5)
    #define PORTB6      (GPIOB->ODR_BITS.bit6)
    #define PORTB7      (GPIOB->ODR_BITS.bit7)
    
    #define PORTC       (GPIOC->ODR)
    #define GPIOC_ODR   (GPIOC->ODR)
    #define PORTC0      (GPIOC->ODR_BITS.bit0)
    #define PORTC1      (GPIOC->ODR_BITS.bit1)
    #define PORTC2      (GPIOC->ODR_BITS.bit2)
    #define PORTC3      (GPIOC->ODR_BITS.bit3)
    #define PORTC4      (GPIOC->ODR_BITS.bit4)
    #define PORTC5      (GPIOC->ODR_BITS.bit5)
    #define PORTC6      (GPIOC->ODR_BITS.bit6)
    #define PORTC7      (GPIOC->ODR_BITS.bit7)
    
    #define PORTD       (GPIOD->ODR)
    #define GPIOD_ODR   (GPIOD->ODR)
    #define PORTD0      (GPIOD->ODR_BITS.bit0)
    #define PORTD1      (GPIOD->ODR_BITS.bit1)
    #define PORTD2      (GPIOD->ODR_BITS.bit2)
    #define PORTD3      (GPIOD->ODR_BITS.bit3)
    #define PORTD4      (GPIOD->ODR_BITS.bit4)
    #define PORTD5      (GPIOD->ODR_BITS.bit5)
    #define PORTD6      (GPIOD->ODR_BITS.bit6)
    #define PORTD7      (GPIOD->ODR_BITS.bit7)
    
    #define PORTE       (GPIOE->ODR)
    #define GPIOE_ODR   (GPIOE->ODR)
    #define PORTE0      (GPIOE->ODR_BITS.bit0)
    #define PORTE1      (GPIOE->ODR_BITS.bit1)
    #define PORTE2      (GPIOE->ODR_BITS.bit2)
    #define PORTE3      (GPIOE->ODR_BITS.bit3)
    #define PORTE4      (GPIOE->ODR_BITS.bit4)
    #define PORTE5      (GPIOE->ODR_BITS.bit5)
    #define PORTE6      (GPIOE->ODR_BITS.bit6)
    #define PORTE7      (GPIOE->ODR_BITS.bit7)
    
    #define PORTF       (GPIOF->ODR)
    #define GPIOF_ODR   (GPIOF->ODR)
    #define PORTF0      (GPIOF->ODR_BITS.bit0)
    #define PORTF1      (GPIOF->ODR_BITS.bit1)
    #define PORTF2      (GPIOF->ODR_BITS.bit2)
    #define PORTF3      (GPIOF->ODR_BITS.bit3)
    #define PORTF4      (GPIOF->ODR_BITS.bit4)
    #define PORTF5      (GPIOF->ODR_BITS.bit5)
    #define PORTF6      (GPIOF->ODR_BITS.bit6)
    #define PORTF7      (GPIOF->ODR_BITS.bit7)
    
    #define PORTG       (GPIOG->ODR)
    #define GPIOG_ODR   (GPIOG->ODR)
    #define PORTG0      (GPIOG->ODR_BITS.bit0)
    #define PORTG1      (GPIOG->ODR_BITS.bit1)
    #define PORTG2      (GPIOG->ODR_BITS.bit2)
    #define PORTG3      (GPIOG->ODR_BITS.bit3)
    #define PORTG4      (GPIOG->ODR_BITS.bit4)
    #define PORTG5      (GPIOG->ODR_BITS.bit5)
    #define PORTG6      (GPIOG->ODR_BITS.bit6)
    #define PORTG7      (GPIOG->ODR_BITS.bit7)
    
    #define PORTH       (GPIOH->ODR)
    #define GPIOH_ODR   (GPIOH->ODR)
    #define PORTH0      (GPIOH->ODR_BITS.bit0)
    #define PORTH1      (GPIOH->ODR_BITS.bit1)
    #define PORTH2      (GPIOH->ODR_BITS.bit2)
    #define PORTH3      (GPIOH->ODR_BITS.bit3)
    #define PORTH4      (GPIOH->ODR_BITS.bit4)
    #define PORTH5      (GPIOH->ODR_BITS.bit5)
    #define PORTH6      (GPIOH->ODR_BITS.bit6)
    #define PORTH7      (GPIOH->ODR_BITS.bit7)
    
    #define PORTI       (GPIOI->ODR)
    #define GPIOI_ODR   (GPIOI->ODR)
    #define PORTI0      (GPIOI->ODR_BITS.bit0)
    #define PORTI1      (GPIOI->ODR_BITS.bit1)
    #define PORTI2      (GPIOI->ODR_BITS.bit2)
    #define PORTI3      (GPIOI->ODR_BITS.bit3)
    #define PORTI4      (GPIOI->ODR_BITS.bit4)
    #define PORTI5      (GPIOI->ODR_BITS.bit5)
    #define PORTI6      (GPIOI->ODR_BITS.bit6)
    #define PORTI7      (GPIOI->ODR_BITS.bit7)
    
    
 // определения входных битов портов IO    
    
    #define PINA       (GPIOA->IDR)
    #define GPIOA_IDR  (GPIOA->IDR)
    #define PINA0      (GPIOA->IDR_BITS.bit0)
    #define PINA1      (GPIOA->IDR_BITS.bit1)
    #define PINA2      (GPIOA->IDR_BITS.bit2)
    #define PINA3      (GPIOA->IDR_BITS.bit3)
    #define PINA4      (GPIOA->IDR_BITS.bit4)
    #define PINA5      (GPIOA->IDR_BITS.bit5)
    #define PINA6      (GPIOA->IDR_BITS.bit6)
    #define PINA7      (GPIOA->IDR_BITS.bit7)
    
    #define PINB       (GPIOB->IDR)
    #define GPIOB_IDR  (GPIOB->IDR)
    #define PINB0      (GPIOB->IDR_BITS.bit0)
    #define PINB1      (GPIOB->IDR_BITS.bit1)
    #define PINB2      (GPIOB->IDR_BITS.bit2)
    #define PINB3      (GPIOB->IDR_BITS.bit3)
    #define PINB4      (GPIOB->IDR_BITS.bit4)
    #define PINB5      (GPIOB->IDR_BITS.bit5)
    #define PINB6      (GPIOB->IDR_BITS.bit6)
    #define PINB7      (GPIOB->IDR_BITS.bit7)
    
    #define PINC       (GPIOC->IDR)
    #define GPIOC_IDR  (GPIOC->IDR)
    #define PINC0      (GPIOC->IDR_BITS.bit0)
    #define PINC1      (GPIOC->IDR_BITS.bit1)
    #define PINC2      (GPIOC->IDR_BITS.bit2)
    #define PINC3      (GPIOC->IDR_BITS.bit3)
    #define PINC4      (GPIOC->IDR_BITS.bit4)
    #define PINC5      (GPIOC->IDR_BITS.bit5)
    #define PINC6      (GPIOC->IDR_BITS.bit6)
    #define PINC7      (GPIOC->IDR_BITS.bit7)
    
    #define PIND       (GPIOD->IDR)
    #define GPIOD_IDR  (GPIOD->IDR)
    #define PIND0      (GPIOD->IDR_BITS.bit0)
    #define PIND1      (GPIOD->IDR_BITS.bit1)
    #define PIND2      (GPIOD->IDR_BITS.bit2)
    #define PIND3      (GPIOD->IDR_BITS.bit3)
    #define PIND4      (GPIOD->IDR_BITS.bit4)
    #define PIND5      (GPIOD->IDR_BITS.bit5)
    #define PIND6      (GPIOD->IDR_BITS.bit6)
    #define PIND7      (GPIOD->IDR_BITS.bit7)
    
    #define PINE       (GPIOE->IDR)
    #define GPIOE_IDR  (GPIOE->IDR)
    #define PINE0      (GPIOE->IDR_BITS.bit0)
    #define PINE1      (GPIOE->IDR_BITS.bit1)
    #define PINE2      (GPIOE->IDR_BITS.bit2)
    #define PINE3      (GPIOE->IDR_BITS.bit3)
    #define PINE4      (GPIOE->IDR_BITS.bit4)
    #define PINE5      (GPIOE->IDR_BITS.bit5)
    #define PINE6      (GPIOE->IDR_BITS.bit6)
    #define PINE7      (GPIOE->IDR_BITS.bit7)
    
    #define PINF       (GPIOF->IDR)
    #define GPIOF_IDR  (GPIOF->IDR)
    #define PINF0      (GPIOF->IDR_BITS.bit0)
    #define PINF1      (GPIOF->IDR_BITS.bit1)
    #define PINF2      (GPIOF->IDR_BITS.bit2)
    #define PINF3      (GPIOF->IDR_BITS.bit3)
    #define PINF4      (GPIOF->IDR_BITS.bit4)
    #define PINF5      (GPIOF->IDR_BITS.bit5)
    #define PINF6      (GPIOF->IDR_BITS.bit6)
    #define PINF7      (GPIOF->IDR_BITS.bit7)
    
    #define PING       (GPIOG->IDR)
    #define GPIOG_IDR  (GPIOG->IDR)
    #define PING0      (GPIOG->IDR_BITS.bit0)
    #define PING1      (GPIOG->IDR_BITS.bit1)
    #define PING2      (GPIOG->IDR_BITS.bit2)
    #define PING3      (GPIOG->IDR_BITS.bit3)
    #define PING4      (GPIOG->IDR_BITS.bit4)
    #define PING5      (GPIOG->IDR_BITS.bit5)
    #define PING6      (GPIOG->IDR_BITS.bit6)
    #define PING7      (GPIOG->IDR_BITS.bit7)
    
    #define PINH       (GPIOH->IDR)
    #define GPIOH_IDR  (GPIOH->IDR)
    #define PINH0      (GPIOH->IDR_BITS.bit0)
    #define PINH1      (GPIOH->IDR_BITS.bit1)
    #define PINH2      (GPIOH->IDR_BITS.bit2)
    #define PINH3      (GPIOH->IDR_BITS.bit3)
    #define PINH4      (GPIOH->IDR_BITS.bit4)
    #define PINH5      (GPIOH->IDR_BITS.bit5)
    #define PINH6      (GPIOH->IDR_BITS.bit6)
    #define PINH7      (GPIOH->IDR_BITS.bit7)
    
    #define PINI       (GPIOI->IDR)
    #define GPIOI_IDR  (GPIOI->IDR)
    #define PINI0      (GPIOI->IDR_BITS.bit0)
    #define PINI1      (GPIOI->IDR_BITS.bit1)
    #define PINI2      (GPIOI->IDR_BITS.bit2)
    #define PINI3      (GPIOI->IDR_BITS.bit3)
    #define PINI4      (GPIOI->IDR_BITS.bit4)
    #define PINI5      (GPIOI->IDR_BITS.bit5)
    #define PINI6      (GPIOI->IDR_BITS.bit6)
    #define PINI7      (GPIOI->IDR_BITS.bit7)
    
    
    
    #define DDRA       (GPIOA->DDR)
    #define GPIOA_DDR  (GPIOA->DDR)
    #define DDRA0      (GPIOA->DDR_BITS.bit0)
    #define DDRA1      (GPIOA->DDR_BITS.bit1)
    #define DDRA2      (GPIOA->DDR_BITS.bit2)
    #define DDRA3      (GPIOA->DDR_BITS.bit3)
    #define DDRA4      (GPIOA->DDR_BITS.bit4)
    #define DDRA5      (GPIOA->DDR_BITS.bit5)
    #define DDRA6      (GPIOA->DDR_BITS.bit6)
    #define DDRA7      (GPIOA->DDR_BITS.bit7)
    
    #define DDRB       (GPIOB->DDR)
    #define GPIOB_DDR  (GPIOB->DDR)
    #define DDRB0      (GPIOB->DDR_BITS.bit0)
    #define DDRB1      (GPIOB->DDR_BITS.bit1)
    #define DDRB2      (GPIOB->DDR_BITS.bit2)
    #define DDRB3      (GPIOB->DDR_BITS.bit3)
    #define DDRB4      (GPIOB->DDR_BITS.bit4)
    #define DDRB5      (GPIOB->DDR_BITS.bit5)
    #define DDRB6      (GPIOB->DDR_BITS.bit6)
    #define DDRB7      (GPIOB->DDR_BITS.bit7)
    
    #define DDRC       (GPIOC->DDR)
    #define GPIOC_DDR  (GPIOC->DDR)
    #define DDRC0      (GPIOC->DDR_BITS.bit0)
    #define DDRC1      (GPIOC->DDR_BITS.bit1)
    #define DDRC2      (GPIOC->DDR_BITS.bit2)
    #define DDRC3      (GPIOC->DDR_BITS.bit3)
    #define DDRC4      (GPIOC->DDR_BITS.bit4)
    #define DDRC5      (GPIOC->DDR_BITS.bit5)
    #define DDRC6      (GPIOC->DDR_BITS.bit6)
    #define DDRC7      (GPIOC->DDR_BITS.bit7)
    
    #define DDRD       (GPIOD->DDR)
    #define GPIOD_DDR  (GPIOD->DDR)
    #define DDRD0      (GPIOD->DDR_BITS.bit0)
    #define DDRD1      (GPIOD->DDR_BITS.bit1)
    #define DDRD2      (GPIOD->DDR_BITS.bit2)
    #define DDRD3      (GPIOD->DDR_BITS.bit3)
    #define DDRD4      (GPIOD->DDR_BITS.bit4)
    #define DDRD5      (GPIOD->DDR_BITS.bit5)
    #define DDRD6      (GPIOD->DDR_BITS.bit6)
    #define DDRD7      (GPIOD->DDR_BITS.bit7)
    
    #define DDRE       (GPIOE->DDR)
    #define GPIOE_DDR  (GPIOE->DDR)
    #define DDRE0      (GPIOE->DDR_BITS.bit0)
    #define DDRE1      (GPIOE->DDR_BITS.bit1)
    #define DDRE2      (GPIOE->DDR_BITS.bit2)
    #define DDRE3      (GPIOE->DDR_BITS.bit3)
    #define DDRE4      (GPIOE->DDR_BITS.bit4)
    #define DDRE5      (GPIOE->DDR_BITS.bit5)
    #define DDRE6      (GPIOE->DDR_BITS.bit6)
    #define DDRE7      (GPIOE->DDR_BITS.bit7)
    
    #define DDRF       (GPIOF->DDR)
    #define GPIOF_DDR  (GPIOF->DDR)
    #define DDRF0      (GPIOF->DDR_BITS.bit0)
    #define DDRF1      (GPIOF->DDR_BITS.bit1)
    #define DDRF2      (GPIOF->DDR_BITS.bit2)
    #define DDRF3      (GPIOF->DDR_BITS.bit3)
    #define DDRF4      (GPIOF->DDR_BITS.bit4)
    #define DDRF5      (GPIOF->DDR_BITS.bit5)
    #define DDRF6      (GPIOF->DDR_BITS.bit6)
    #define DDRF7      (GPIOF->DDR_BITS.bit7)
    
    #define DDRG       (GPIOG->DDR)
    #define GPIOG_DDR  (GPIOG->DDR)
    #define DDRG0      (GPIOG->DDR_BITS.bit0)
    #define DDRG1      (GPIOG->DDR_BITS.bit1)
    #define DDRG2      (GPIOG->DDR_BITS.bit2)
    #define DDRG3      (GPIOG->DDR_BITS.bit3)
    #define DDRG4      (GPIOG->DDR_BITS.bit4)
    #define DDRG5      (GPIOG->DDR_BITS.bit5)
    #define DDRG6      (GPIOG->DDR_BITS.bit6)
    #define DDRG7      (GPIOG->DDR_BITS.bit7)
    
    #define DDRH       (GPIOH->DDR)
    #define GPIOH_DDR  (GPIOH->DDR)
    #define DDRH0      (GPIOH->DDR_BITS.bit0)
    #define DDRH1      (GPIOH->DDR_BITS.bit1)
    #define DDRH2      (GPIOH->DDR_BITS.bit2)
    #define DDRH3      (GPIOH->DDR_BITS.bit3)
    #define DDRH4      (GPIOH->DDR_BITS.bit4)
    #define DDRH5      (GPIOH->DDR_BITS.bit5)
    #define DDRH6      (GPIOH->DDR_BITS.bit6)
    #define DDRH7      (GPIOH->DDR_BITS.bit7)
    
    #define DDRI       (GPIOI->DDR)
    #define GPIOI_DDR  (GPIOI->DDR)
    #define DDRI0      (GPIOI->DDR_BITS.bit0)
    #define DDRI1      (GPIOI->DDR_BITS.bit1)
    #define DDRI2      (GPIOI->DDR_BITS.bit2)
    #define DDRI3      (GPIOI->DDR_BITS.bit3)
    #define DDRI4      (GPIOI->DDR_BITS.bit4)
    #define DDRI5      (GPIOI->DDR_BITS.bit5)
    #define DDRI6      (GPIOI->DDR_BITS.bit6)
    #define DDRI7      (GPIOI->DDR_BITS.bit7)
    
    



    #define CR1A       (GPIOA->CR1)
    #define GPIOA_CR1  (GPIOA->CR1)
    #define CR1A0      (GPIOA->CR1_BITS.bit0)
    #define CR1A1      (GPIOA->CR1_BITS.bit1)
    #define CR1A2      (GPIOA->CR1_BITS.bit2)
    #define CR1A3      (GPIOA->CR1_BITS.bit3)
    #define CR1A4      (GPIOA->CR1_BITS.bit4)
    #define CR1A5      (GPIOA->CR1_BITS.bit5)
    #define CR1A6      (GPIOA->CR1_BITS.bit6)
    #define CR1A7      (GPIOA->CR1_BITS.bit7)
    
    #define CR1B       (GPIOB->CR1)
    #define GPIOB_CR1  (GPIOB->CR1)
    #define CR1B0      (GPIOB->CR1_BITS.bit0)
    #define CR1B1      (GPIOB->CR1_BITS.bit1)
    #define CR1B2      (GPIOB->CR1_BITS.bit2)
    #define CR1B3      (GPIOB->CR1_BITS.bit3)
    #define CR1B4      (GPIOB->CR1_BITS.bit4)
    #define CR1B5      (GPIOB->CR1_BITS.bit5)
    #define CR1B6      (GPIOB->CR1_BITS.bit6)
    #define CR1B7      (GPIOB->CR1_BITS.bit7)
	
    #define CR1C       (GPIOC->CR1)
    #define GPIOC_CR1  (GPIOC->CR1)
    #define CR1C0      (GPIOC->CR1_BITS.bit0)
    #define CR1C1      (GPIOC->CR1_BITS.bit1)
    #define CR1C2      (GPIOC->CR1_BITS.bit2)
    #define CR1C3      (GPIOC->CR1_BITS.bit3)
    #define CR1C4      (GPIOC->CR1_BITS.bit4)
    #define CR1C5      (GPIOC->CR1_BITS.bit5)
    #define CR1C6      (GPIOC->CR1_BITS.bit6)
    #define CR1C7      (GPIOC->CR1_BITS.bit7)
	
    #define CR1D       (GPIOD->CR1)
    #define GPIOD_CR1  (GPIOD->CR1)
    #define CR1D0      (GPIOD->CR1_BITS.bit0)
    #define CR1D1      (GPIOD->CR1_BITS.bit1)
    #define CR1D2      (GPIOD->CR1_BITS.bit2)
    #define CR1D3      (GPIOD->CR1_BITS.bit3)
    #define CR1D4      (GPIOD->CR1_BITS.bit4)
    #define CR1D5      (GPIOD->CR1_BITS.bit5)
    #define CR1D6      (GPIOD->CR1_BITS.bit6)
    #define CR1D7      (GPIOD->CR1_BITS.bit7)
	
    #define CR1E       (GPIOE->CR1)
    #define GPIOE_CR1  (GPIOE->CR1)
    #define CR1E0      (GPIOE->CR1_BITS.bit0)
    #define CR1E1      (GPIOE->CR1_BITS.bit1)
    #define CR1E2      (GPIOE->CR1_BITS.bit2)
    #define CR1E3      (GPIOE->CR1_BITS.bit3)
    #define CR1E4      (GPIOE->CR1_BITS.bit4)
    #define CR1E5      (GPIOE->CR1_BITS.bit5)
    #define CR1E6      (GPIOE->CR1_BITS.bit6)
    #define CR1E7      (GPIOE->CR1_BITS.bit7)
	
    #define CR1F       (GPIOF->CR1)
    #define GPIOF_CR1  (GPIOF->CR1)
    #define CR1F0      (GPIOF->CR1_BITS.bit0)
    #define CR1F1      (GPIOF->CR1_BITS.bit1)
    #define CR1F2      (GPIOF->CR1_BITS.bit2)
    #define CR1F3      (GPIOF->CR1_BITS.bit3)
    #define CR1F4      (GPIOF->CR1_BITS.bit4)
    #define CR1F5      (GPIOF->CR1_BITS.bit5)
    #define CR1F6      (GPIOF->CR1_BITS.bit6)
    #define CR1F7      (GPIOF->CR1_BITS.bit7)
	
    #define CR1G       (GPIOG->CR1)
    #define GPIOG_CR1  (GPIOG->CR1)
    #define CR1G0      (GPIOG->CR1_BITS.bit0)
    #define CR1G1      (GPIOG->CR1_BITS.bit1)
    #define CR1G2      (GPIOG->CR1_BITS.bit2)
    #define CR1G3      (GPIOG->CR1_BITS.bit3)
    #define CR1G4      (GPIOG->CR1_BITS.bit4)
    #define CR1G5      (GPIOG->CR1_BITS.bit5)
    #define CR1G6      (GPIOG->CR1_BITS.bit6)
    #define CR1G7      (GPIOG->CR1_BITS.bit7)
	
    #define CR1H       (GPIOH->CR1)
    #define GPIOH_CR1  (GPIOH->CR1)
    #define CR1H0      (GPIOH->CR1_BITS.bit0)
    #define CR1H1      (GPIOH->CR1_BITS.bit1)
    #define CR1H2      (GPIOH->CR1_BITS.bit2)
    #define CR1H3      (GPIOH->CR1_BITS.bit3)
    #define CR1H4      (GPIOH->CR1_BITS.bit4)
    #define CR1H5      (GPIOH->CR1_BITS.bit5)
    #define CR1H6      (GPIOH->CR1_BITS.bit6)
    #define CR1H7      (GPIOH->CR1_BITS.bit7)
	
    #define CR1I       (GPIOI->CR1)
    #define GPIOI_CR1  (GPIOI->CR1)
    #define CR1I0      (GPIOI->CR1_BITS.bit0)
    #define CR1I1      (GPIOI->CR1_BITS.bit1)
    #define CR1I2      (GPIOI->CR1_BITS.bit2)
    #define CR1I3      (GPIOI->CR1_BITS.bit3)
    #define CR1I4      (GPIOI->CR1_BITS.bit4)
    #define CR1I5      (GPIOI->CR1_BITS.bit5)
    #define CR1I6      (GPIOI->CR1_BITS.bit6)
    #define CR1I7      (GPIOI->CR1_BITS.bit7)



	
    #define CR2A       (GPIOA->CR2)
    #define GPIOA_CR2  (GPIOA->CR2)
    #define CR2A0      (GPIOA->CR2_BITS.bit0)
    #define CR2A1      (GPIOA->CR2_BITS.bit1)
    #define CR2A2      (GPIOA->CR2_BITS.bit2)
    #define CR2A3      (GPIOA->CR2_BITS.bit3)
    #define CR2A4      (GPIOA->CR2_BITS.bit4)
    #define CR2A5      (GPIOA->CR2_BITS.bit5)
    #define CR2A6      (GPIOA->CR2_BITS.bit6)
    #define CR2A7      (GPIOA->CR2_BITS.bit7)
 	
    #define CR2B       (GPIOB->CR2)
    #define GPIOB_CR2  (GPIOB->CR2)
    #define CR2B0      (GPIOB->CR2_BITS.bit0)
    #define CR2B1      (GPIOB->CR2_BITS.bit1)
    #define CR2B2      (GPIOB->CR2_BITS.bit2)
    #define CR2B3      (GPIOB->CR2_BITS.bit3)
    #define CR2B4      (GPIOB->CR2_BITS.bit4)
    #define CR2B5      (GPIOB->CR2_BITS.bit5)
    #define CR2B6      (GPIOB->CR2_BITS.bit6)
    #define CR2B7      (GPIOB->CR2_BITS.bit7)
	
    #define CR2C       (GPIOC->CR2)
    #define GPIOC_CR2  (GPIOC->CR2)
    #define CR2C0      (GPIOC->CR2_BITS.bit0)
    #define CR2C1      (GPIOC->CR2_BITS.bit1)
    #define CR2C2      (GPIOC->CR2_BITS.bit2)
    #define CR2C3      (GPIOC->CR2_BITS.bit3)
    #define CR2C4      (GPIOC->CR2_BITS.bit4)
    #define CR2C5      (GPIOC->CR2_BITS.bit5)
    #define CR2C6      (GPIOC->CR2_BITS.bit6)
    #define CR2C7      (GPIOC->CR2_BITS.bit7)
	
    #define CR2D       (GPIOD->CR2)
    #define GPIOD_CR2  (GPIOD->CR2)
    #define CR2D0      (GPIOD->CR2_BITS.bit0)
    #define CR2D1      (GPIOD->CR2_BITS.bit1)
    #define CR2D2      (GPIOD->CR2_BITS.bit2)
    #define CR2D3      (GPIOD->CR2_BITS.bit3)
    #define CR2D4      (GPIOD->CR2_BITS.bit4)
    #define CR2D5      (GPIOD->CR2_BITS.bit5)
    #define CR2D6      (GPIOD->CR2_BITS.bit6)
    #define CR2D7      (GPIOD->CR2_BITS.bit7)
	
    #define CR2E       (GPIOE->CR2)
    #define GPIOE_CR2  (GPIOE->CR2)
    #define CR2E0      (GPIOE->CR2_BITS.bit0)
    #define CR2E1      (GPIOE->CR2_BITS.bit1)
    #define CR2E2      (GPIOE->CR2_BITS.bit2)
    #define CR2E3      (GPIOE->CR2_BITS.bit3)
    #define CR2E4      (GPIOE->CR2_BITS.bit4)
    #define CR2E5      (GPIOE->CR2_BITS.bit5)
    #define CR2E6      (GPIOE->CR2_BITS.bit6)
    #define CR2E7      (GPIOE->CR2_BITS.bit7)
	
    #define CR2F       (GPIOF->CR2)
    #define GPIOF_CR2  (GPIOF->CR2)
    #define CR2F0      (GPIOF->CR2_BITS.bit0)
    #define CR2F1      (GPIOF->CR2_BITS.bit1)
    #define CR2F2      (GPIOF->CR2_BITS.bit2)
    #define CR2F3      (GPIOF->CR2_BITS.bit3)
    #define CR2F4      (GPIOF->CR2_BITS.bit4)
    #define CR2F5      (GPIOF->CR2_BITS.bit5)
    #define CR2F6      (GPIOF->CR2_BITS.bit6)
    #define CR2F7      (GPIOF->CR2_BITS.bit7)
	
    #define CR2G       (GPIOG->CR2)
    #define GPIOG_CR2  (GPIOG->CR2)
    #define CR2G0      (GPIOG->CR2_BITS.bit0)
    #define CR2G1      (GPIOG->CR2_BITS.bit1)
    #define CR2G2      (GPIOG->CR2_BITS.bit2)
    #define CR2G3      (GPIOG->CR2_BITS.bit3)
    #define CR2G4      (GPIOG->CR2_BITS.bit4)
    #define CR2G5      (GPIOG->CR2_BITS.bit5)
    #define CR2G6      (GPIOG->CR2_BITS.bit6)
    #define CR2G7      (GPIOG->CR2_BITS.bit7)
	
    #define CR2H       (GPIOH->CR2)
    #define GPIOH_CR2  (GPIOH->CR2)
    #define CR2H0      (GPIOH->CR2_BITS.bit0)
    #define CR2H1      (GPIOH->CR2_BITS.bit1)
    #define CR2H2      (GPIOH->CR2_BITS.bit2)
    #define CR2H3      (GPIOH->CR2_BITS.bit3)
    #define CR2H4      (GPIOH->CR2_BITS.bit4)
    #define CR2H5      (GPIOH->CR2_BITS.bit5)
    #define CR2H6      (GPIOH->CR2_BITS.bit6)
    #define CR2H7      (GPIOH->CR2_BITS.bit7)
	
    #define CR2I       (GPIOI->CR2)
    #define GPIOI_CR2  (GPIOI->CR2)
    #define CR2I0      (GPIOI->CR2_BITS.bit0)
    #define CR2I1      (GPIOI->CR2_BITS.bit1)
    #define CR2I2      (GPIOI->CR2_BITS.bit2)
    #define CR2I3      (GPIOI->CR2_BITS.bit3)
    #define CR2I4      (GPIOI->CR2_BITS.bit4)
    #define CR2I5      (GPIOI->CR2_BITS.bit5)
    #define CR2I6      (GPIOI->CR2_BITS.bit6)
    #define CR2I7      (GPIOI->CR2_BITS.bit7)
	
	
   
#pragma inline = forced 
	void GPIO_ConfigOutput(gpio_t *gpio, uint8_t pin, OutputSpeed_t speed, OutputType_t type)
    {
        gpio->DDR |= (uint8_t)(1<<(pin & 0x07));
        if(type == PortOpenDrain) 
		{
			gpio->CR1 &= (uint8_t)(~(1<<(pin & 0x07)));
		}
        else 
		{
			gpio->CR1 |= (uint8_t)(1<<(pin & 0x07));
        ...
        if(speed == PortLoSpeed) 
		{
			gpio->CR2 &= (uint8_t)(~(1<<(pin & 0x07)));
		}
        else 
		{
			gpio->CR2 |= (uint8_t)(1<<(pin & 0x07));
...
#pragma inline = forced 
	void GPIO_ConfigInput(gpio_t *gpio, uint8_t pin, InputType_t type, InputIrq_t  irq)
    {
		if(irq == PinIrqOff) 
		{
			gpio->CR2 &= (uint8_t)(~(1<<(pin & 0x07)));                
...
#endif /* Для совместимости с внешней библиотекой stm8s_gpio.h */
